# 문제
# 평면에 네 개의 직사각형이 놓여 있는데 그 밑변은 모두 가로축에 평행하다. 
# 이 네 개의 직사각형들은 서로 떨어져 있을 수도 있고, 겹쳐 있을 수도 있고, 
# 하나가 다른 하나를 포함할 수도 있으며, 변이나 꼭짓점이 겹칠 수도 있다.
# 이 직사각형들이 차지하는 면적을 구하는 프로그램을 작성하시오.

# 입력
# 입력은 네 줄이며, 각 줄은 직사각형의 위치를 나타내는 네 개의 정수로 주어진다. 
# 첫 번째와 두 번째의 정수는 사각형의 왼쪽 아래 꼭짓점의 x좌표, y좌표이고 
# 세 번째와 네 번째의 정수는 사각형의 오른쪽 위 꼭짓점의 x좌표, y좌표이다. 
# 모든 x좌표와 y좌표는 1이상이고 100이하인 정수이다.

paper = [[0 for _ in range(100)] for _ in range(100)]
# 먼저 0으로 이루어진 100 X 100 의 필드를 준비해준다.

for _ in range(4):
    x, y, x2, y2 = map(int,input().split())
    # 사각형의 x 시작좌표와 x끝 좌표, y시작 좌표와 y끝 좌표를 받아온다.
    
    for i in range(x, x2):
        for j in range(y, y2):
            paper[i][j] = 1
            # 필드에서 지정된 좌표 내의 0을 전부 1로 변경해준다.

total = 0
# 넓이를 의미하는 total 변수를 지정해준다.

for line in paper:
    # paper 2차원 리스트의 인자인 하위 리스트 내의 값을 총합을 낸다.
    total += sum(line)
    # 총합산을 total에 추가해 준다.

print(total)
