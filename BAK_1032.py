...
a, b = input(), [input() for _ in range(N-1)]
empty = ''
...
for i, char in enumerate(a):
    for j in b:
        if char != j[i]:
            empty += '?'
            break            
            # 만약 첫번째 문자열인 a의 i번째 단어 가 b리스트의 j번째 문자열의 i번 단어와 
            # 다르다면, '?'를 추가해준다.
    else:
        empty += char
            # 만약 첫번째 문자열인 a의 i번째 단어 가 b리스트의 j번째 문자열의 i번 단어와 다르지 않다면,
            # 해당 단어를 c에 추가해준다.
            
            # 예시 1번
            # 처음 들어온 문자열 config.sys의 첫단어는 c이고
            # b 리스트의 0번째 문자열 'config.inf'의 첫 단어도 c 이고, 
            # 두번째 문자열 'configures'의 첫 단어도 c이다!
            # 따라서 공백 문자열 empty 에 c를 넣어준다.
