...
# 먼저 소요 시간과 다이얼에 쓰여진 단어를 짝지어 딕셔너리를 만들어 준다.
dial = {
    'ABC' : 3,
    'DEF' : 4,
    'GHI' : 5,
...
# 총소요시간을 미리 0초로 지정해 준다.
total_time = 0

for word in a:
    for key in dial:
        if word in key:
            # 입력받은 a의 단어중에 dial 의 key값에 포함되는 단어가 있다면 해당 key 값의 value를 가져와
            # 총소요시간(total_time)에 더해 총 소요시간을 구한다.
            total_time += dial[key]
